...
const ensureOperationalForAvailable = async (slot: {
  ...
  if (slot.endAt <= new Date()) throw new ConflictError("Khung giờ đã qua và không thể bật lại.");
...
export const bayTimeSlotService = {
  createBulk: async (stationId: string, input: BulkCreateInput, adminId: string) => {
    ...
    await prisma.station.update({
      ...
      data: {
        ...
        baySlotBufferMinutes: input.bufferMinutes,
      },
    });
    await prisma.bayTimeSlot.updateMany({
      where: {
        stationId,
        endAt: { gt: now },
        status: {
          notIn: [BayTimeSlotStatus.CANCELLED, BayTimeSlotStatus.EXPIRED],
        },
      },
  ...
  list: async (stationId: string, query: {
    ...
    await bayTimeSlotService.releaseCompletedCooldowns(new Date(), stationId);
    const station = await prisma.station.findUnique({
      where: { id: stationId },
      select: { id: true, baySlotBufferMinutes: true },
    });
    if (!station) throw new NotFoundError("Không tìm thấy trạm.");
    const bayWhere = {
      stationId,
    ...
    for (const slot of slots) {
      if (
        slot.status !== BayTimeSlotStatus.COMPLETED
        || slot.cooldownEndsAt
        || slot.endAt <= now
      ) continue;

      const sameBaySlots = slots.filter((candidate) => candidate.bayId === slot.bayId);
      const slotIndex = sameBaySlots.findIndex((candidate) => candidate.id === slot.id);
      const previousSlot = slotIndex > 0 ? sameBaySlots[slotIndex - 1] : undefined;
      const nextSlot = slotIndex >= 0 ? sameBaySlots[slotIndex + 1] : undefined;
      const inferredBufferMinutes = nextSlot
        ? Math.max(0, Math.round((nextSlot.startAt.getTime() - slot.endAt.getTime()) / 60_000))
        : previousSlot
          ? Math.max(0, Math.round((slot.startAt.getTime() - previousSlot.endAt.getTime()) / 60_000))
          : 0;
      const bufferMinutes =
        slot.bufferMinutes ?? station.baySlotBufferMinutes ?? inferredBufferMinutes;
      const cooldownEndsAt = calculateBaySlotCooldownEndsAt(
        slot.updatedAt,
        slot.endAt,
        bufferMinutes,
      );
      if (!cooldownEndsAt) continue;

      if (cooldownEndsAt <= now) {
        await prisma.bayTimeSlot.update({
          where: { id: slot.id },
          data: {
        ...
        slot.bookingId = null;
        slot.bufferMinutes = bufferMinutes;
      } else {
        await prisma.bayTimeSlot.update({
          where: { id: slot.id },
          data: { bufferMinutes, cooldownEndsAt },
        });
        slot.bufferMinutes = bufferMinutes;
        slot.cooldownEndsAt = cooldownEndsAt;
      }
    }
    const visibleSlots = query.status
      ? slots.filter((slot) => slot.status === query.status)
      : slots;
    return {
      date: query.date,
      bays: bays.map((bay) => ({
        ...
        slots: visibleSlots.filter((slot) => slot.bayId === bay.id).map(serialize),
      })),
      pagination: {
        page: query.page,
        limit: query.limit,
  ...
  updateStatus: async (slotId: string, input: StatusInput, adminId: string) => {
    ...
    const attemptsManualOverride =
      (before.status === BayTimeSlotStatus.RESERVED
        && (next === BayTimeSlotStatus.AVAILABLE
          || next === BayTimeSlotStatus.COMPLETED))
      || (before.status === BayTimeSlotStatus.COMPLETED
        && next === BayTimeSlotStatus.AVAILABLE);
    if (attemptsManualOverride && before.bookingId) {
      throw new ConflictError("Slot có booking thật phải được xử lý trong quy trình của khách hàng.");
    }

    const completedAt = new Date();
    const stationBuffer = next === BayTimeSlotStatus.COMPLETED && before.bufferMinutes === null
      ? await prisma.station.findUnique({
        where: { id: before.stationId },
        select: { baySlotBufferMinutes: true },
      })
      : null;
    const bufferMinutes = before.bufferMinutes ?? stationBuffer?.baySlotBufferMinutes ?? 0;
    const cooldownEndsAt = next === BayTimeSlotStatus.COMPLETED
      ? calculateBaySlotCooldownEndsAt(completedAt, before.endAt, bufferMinutes)
      : null;
    const persistedStatus =
      next === BayTimeSlotStatus.COMPLETED
        ...
        : next;
    if (persistedStatus === BayTimeSlotStatus.AVAILABLE) {
      await ensureOperationalForAvailable(before);
    }
    ...
    const slot = await prisma.bayTimeSlot.update({
      ...
      data: {
        ...
        bufferMinutes,
        cooldownEndsAt: persistedStatus === BayTimeSlotStatus.COMPLETED
          ? cooldownEndsAt
          : null,
    ...
    const action =
      ...
      : next === BayTimeSlotStatus.RESERVED ? "BAY_SLOT_RESERVED_MANUALLY"
      : next === BayTimeSlotStatus.COMPLETED ? "BAY_SLOT_WALK_IN_COMPLETED"
      : before.status === BayTimeSlotStatus.COMPLETED ? "BAY_SLOT_WALK_IN_REOPENED"
  ...
  releaseCompletedCooldowns: async (now = new Date(), stationId?: string) => {
    const released = await prisma.bayTimeSlot.updateMany({
      where: {
        ...(stationId ? { stationId } : {}),
