...
const assertStaffScope = async (
  ...
  if (!allowedAssignment)
    throw new ForbiddenError(
      requireCurrentShift
        ? "Bạn hiện không trong ca làm việc được phân công tại trạm này"
        : "Staff is not assigned to this station",
    );
  return allowedAssignment;
};

const assertBookingInsideAssignedShift = (
  assignment: Awaited<ReturnType<typeof assertStaffScope>>,
  booking: {
  ...
  if (
    ...
    throw new ForbiddenError(
      "Lịch đặt này nằm ngoài ca làm việc được phân công của bạn",
    );
  }
};
...
const getContext = async (staffId: string, role: string) => {
  ...
  if (role === Roles.ADMIN) {
    ...
    return {
      ...
      activeSwap: null,
      assignmentState: "ACTIVE_SHIFT" as const,
    };
  }
  const assignments = await prisma.stationAssignment.findMany({
    where: {
  ...
  const effectiveAssignments = assignments.filter((item) =>
    isStationAssignmentEffectiveNow(item, now),
  );
  const stationIds = [
    ...new Set(
      effectiveAssignments
        .filter(
          (item) => isStationAssignmentEffectiveNow(item, now, true),
        )
  ...
  const stations = stationRows.map((station) => {
    const assignment = effectiveAssignments.find(
        (assignment) =>
          assignment.stationId === station.id &&
          isStationAssignmentEffectiveNow(assignment, now, true),
      );
    return {
      ...station,
      assignedShift: assignment?.shift ?? null,
      serviceBays: station.serviceBays.map((bay) => ({
        ...bay,
        bookings: bay.bookings.filter((booking) =>
          isBookingWithinStationAssignmentShift(
            assignment?.shift,
            booking.scheduledStart,
            booking.scheduledEnd,
  ...
  return {
    stations,
    activeSwap,
    assignmentState: effectiveAssignments.length
      ? stationIds.length
...
const lookup = async (
  ...
  const assignment = await assertStaffScope(staffId, role, stationId, true);
  ...
  assertBookingInsideAssignedShift(assignment, booking);
  return booking;
};
