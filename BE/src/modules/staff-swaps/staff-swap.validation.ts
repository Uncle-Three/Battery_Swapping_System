...
export const serialSchema = z.object({ serialNumber: z.string().trim().min(3).max(100), soc: z.number().int().min(0).max(100).optional(), soh: z.number().min(0).max(100).optional() }).strict();
