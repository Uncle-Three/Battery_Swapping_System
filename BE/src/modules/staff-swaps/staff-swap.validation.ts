...
export const serialSchema = z.object({ serialNumber: z.string().trim().min(3).max(100), soc: z.number().int().min(0).max(100).optional() }).strict();
export const scanBatterySchema = z.object({ serialNumber: z.string().trim().min(3).max(100) }).strict();
export const inspectionSchema = z.object({
  serialNumber: z.string().trim().min(3).max(100), soc: z.number().int().min(0).max(100), soh: z.number().min(0).max(100),
...
export const collectPaymentSchema = z.object({}).strict();
