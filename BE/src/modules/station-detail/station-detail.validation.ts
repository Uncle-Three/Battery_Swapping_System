...
export const bookingParamSchema = z.object({ stationId: objectIdSchema, bookingId: objectIdSchema });

export const createBaySchema = z.object({
  bayCode: z.string().trim().regex(/^E\d{7}$/, "Bay code must use the format E followed by 7 digits"), bayName: z.string().trim().min(1).max(100),
...
const time = z.string().regex(
  ...
  "Time must use HH:mm format between 00:00 and 24:00",
);
...
export const inventoryQuerySchema = z.object({ search: z.string().optional(), status: z.string().optional(), safetyState: z.string().optional(), batteryType: z.string().optional(), vehicleModel: z.string().optional(), page: z.coerce.number().int().min(1).default(1), limit: z.coerce.number().int().min(1).max(100).default(20) });
...
export const inventoryActionSchema = z.object({
  action: z.enum(["ADD", "TRANSFER", "INSPECTION_REQUIRED", "MAINTENANCE", "RETIRE", "DELETE"]),
...
export const bookingsQuerySchema = z.object({ search: z.string().trim().max(120).optional(), status: z.string().optional(), batteryType: z.string().trim().max(80).optional(), from: z.coerce.date().optional(), to: z.coerce.date().optional(), page: z.coerce.number().int().min(1).default(1), limit: z.coerce.number().int().min(1).max(100).default(20) });
export const cancelBookingSchema = z.object({
  reason: z.string().trim().min(3).max(500),
});

export const createAssignmentSchema = z.object({ userId: objectIdSchema, assignmentRole: z.literal("STAFF"), shift: z.string().trim().max(100).optional().nullable(), effectiveFrom: z.coerce.date().optional(), effectiveTo: z.coerce.date().optional() });
