...
import {
  auditQuerySchema, bookingsQuerySchema, createAssignmentSchema, createBaySchema,
...
describe("station detail request validation", () => {
  ...
  it("accepts a valid service bay", () => expect(createBaySchema.safeParse({ bayCode: "B01", bayName: "Khoang 1", defaultDurationMinutes: 30 }).success).toBe(true));
  it("rejects a bay with an invalid duration", () => expect(createBaySchema.safeParse({ bayCode: "B01", bayName: "Khoang 1", defaultDurationMinutes: 0 }).success).toBe(false));
