...
const labels = ['Tổng quan','Khoang & Lịch thay pin','Kho pin thay thế','Booking','Nhân sự & Phân công','Bảo trì & Sự cố','Báo cáo','Nhật ký hệ thống'];
...
test('admin station detail has exactly eight persistent real-data tabs', async ({ page }) => {
  ...
  const stationNav = page.locator('nav').filter({ hasText: 'Khoang & Lịch thay pin' });
  ...
  const slotDate = new Date(Date.now() + 400 * 86_400_000).toISOString().slice(0, 10);
  await page.locator('section').filter({ hasText: 'Lịch thay pin' }).locator('input[type="date"]').first().fill(slotDate);
  await page.getByRole('button', { name: 'Tạo lịch' }).click();
  const slotDialog = page.getByRole('heading', { name: 'Tạo lịch thay pin' }).locator('../..');
  await slotDialog.locator('select').selectOption({ label: `${bayCode} · ${bayName}` });
  await slotDialog.locator('input[type="time"]').nth(0).fill('10:00');
  await slotDialog.locator('input[type="time"]').nth(1).fill('10:30');
  await slotDialog.getByRole('button', { name: 'Lưu' }).click();
