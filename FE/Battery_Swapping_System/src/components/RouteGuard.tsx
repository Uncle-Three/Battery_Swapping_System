...
const dashboardOnlyRoles: UserRole[] = ['ADMIN', 'MANAGER', 'STAFF'];

export const DashboardOnlyRoute: FC<ChildrenProps> = ({ children }) => {
  const { isAuthenticated, user } = useAuth();
  const location = useLocation();

  if (!isAuthenticated || !user || !dashboardOnlyRoles.includes(user.role)) {
    return <>{children}</>;
  }

  // VNPay must return through this public callback before an operator can be
  // redirected back to the relevant swap inside the staff dashboard.
  if (location.pathname === '/payments/vnpay/return') {
    return <>{children}</>;
  }

  const dashboard = roleDashboard(user.role);
  const dashboardRoot = dashboard.slice(0, dashboard.lastIndexOf('/'));
  const isInsideDashboard = location.pathname === dashboardRoot
    || location.pathname.startsWith(`${dashboardRoot}/`);

  return isInsideDashboard ? <>{children}</> : <Navigate to={dashboard} replace />;
};
