...
export const BaySlotPreviewMatrix: FC<{
  ...
  const cooldownEndsAtValues = useMemo(
    () => slots
      .filter((slot) => slot.status === 'COMPLETED' && slot.cooldownEndsAt)
      .map((slot) => new Date(slot.cooldownEndsAt!).getTime())
  ...
  useEffect(() => {
    if (!cooldownEndsAtValues.some((value) => value > Date.now())) return;
    const timer = window.setInterval(() => setNowMs(Date.now()), 1000);
    return () => window.clearInterval(timer);
  }, [cooldownEndsAtValues]);

  useEffect(() => {
    if (!onRetry || !cooldownEndsAtValues.length) return;
    const nextReleaseAt = Math.min(...cooldownEndsAtValues.filter((value) => value > Date.now()));
    if (!Number.isFinite(nextReleaseAt)) return;
    const timer = window.setTimeout(onRetry, Math.max(0, nextReleaseAt - Date.now()) + 100);
    return () => window.clearTimeout(timer);
  }, [cooldownEndsAtValues, onRetry]);
  ...
  return (
    <section className="isolate min-w-0 overflow-hidden rounded-[18px] border border-emerald-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      {loading ? (
        ...
        <>
          ...
          ) : <div className="relative isolate max-h-[650px] overflow-auto">
            <table className="relative w-max min-w-full border-separate border-spacing-0 text-left">
              <thead className="sticky top-0 z-30 bg-slate-50 dark:bg-slate-800">
                <tr>
                  <th className="sticky left-0 z-50 min-w-48 overflow-hidden border-b border-r border-slate-200 bg-slate-50 px-4 py-3 text-xs font-black text-slate-600 shadow-[6px_0_8px_-8px_rgba(15,23,42,0.45)] dark:border-slate-700 dark:bg-slate-800">Khoang</th>
                  {ranges.map((range) => (
              ...
              <tbody>
                {bays.map((bay) => (
                  <tr key={bay.id} className="group">
                    <th className="sticky left-0 z-40 overflow-hidden border-b border-r border-slate-100 bg-white px-4 py-3 shadow-[6px_0_8px_-8px_rgba(15,23,42,0.45)] group-hover:bg-emerald-50 dark:border-slate-800 dark:bg-slate-900 dark:group-hover:bg-slate-800">
                    ...
                    {ranges.map((range) => {
                      ...
                      return (
                        <td key={`${range.startTime}:${range.endTime}`} className="relative z-0 border-b border-slate-100 px-3 py-2.5 text-center dark:border-slate-800">
                          {slot ? (
                            <BaySlotStatusBadge
                              status={slot.status}
