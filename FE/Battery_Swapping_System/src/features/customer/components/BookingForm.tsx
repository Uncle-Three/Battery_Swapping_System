...
export const BookingForm: FC<BookingFormProps> = ({ station, onSubmit, onCancel }) => {
  ...
  useEffect(() => {
    bookingService.getMyVehicles().then((items) => {
      const activeVehicles = items.filter((item) => item.status === 'ACTIVE');
      setVehicles(activeVehicles);
      setVehicleId(activeVehicles[0]?.id ?? '');
      if (!activeVehicles.length) setError('Không có xe đang hoạt động. Hãy bật lại xe trước khi đặt lịch thay pin.');
