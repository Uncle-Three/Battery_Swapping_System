...
export const StationCard: FC<StationCardProps> = ({ station, onSelect }) => {
  const readyBatteriesCount = station.slots.filter((slot) => slot.status === 'READY').length;
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className="text-sm">
            Số pin sẵn sàng: <strong className="text-green-600 font-semibold">{readyBatteriesCount}</strong> / {station.slots.length} slots
          </span>
        </div>
      </div>
