      ...
      <section className="app-panel p-6">
        ...
        {terminal ? (
          <div className="mt-4 rounded-xl border border-slate-200/80 bg-slate-100 p-4 font-bold text-slate-700 dark:border-slate-800 dark:bg-slate-800 dark:text-slate-300">
            Kết thúc: {statusObj.label}
            {(item!.cancellation?.reason || item!.rejectionReason)
              ? ` — Lý do: ${item!.cancellation?.reason || item!.rejectionReason}`
              : ''}
          </div>
        ) : (
        ...
        <div className="mt-8 flex flex-col gap-4 relative pl-2">
          ...
          {item!.cancellation && (
            <div className="relative pl-6">
              <div className="absolute left-0 top-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-rose-500/20 ring-4 ring-white dark:ring-slate-900">
              ...
              <p className="text-sm font-extrabold text-rose-700 dark:text-rose-400">
                Hủy bởi <span className="text-slate-900 dark:text-white">{item!.cancellation.cancelledBy?.fullName || 'Quản trị viên'}</span>
              </p>
              <p className="mt-0.5 text-xs font-bold text-rose-600 dark:text-rose-400">
                {new Date(item!.cancellation.cancelledAt).toLocaleString('vi-VN')}
              </p>
              <p className="mt-1.5 rounded-lg bg-rose-50 p-3 text-sm text-rose-800 dark:bg-rose-950/30 dark:text-rose-300">
                Lý do: {item!.cancellation.reason || 'Không có lý do được ghi nhận'}
              </p>
            </div>
          )}
          ...
          {!item!.approvalHistory?.length && !item!.cancellation && !item!.transactions?.length && (
