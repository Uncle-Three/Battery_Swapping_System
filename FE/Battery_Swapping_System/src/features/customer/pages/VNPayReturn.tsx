  ...
  return (
    <div className="mx-auto max-w-lg space-y-6 py-10 text-center">
      ...
      {result?.success && (
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-slate-500">
            VNPay sẽ xác nhận giao dịch qua IPN. Trạng thái booking và lịch sử thay pin sẽ được cập nhật tự động sau khi máy chủ xác minh thành công.
          </p>
        </div>
      )}
      ...
      <div className="flex justify-center gap-3 pt-2">
        ...
        <Link to={result?.bookingId ? `/app/bookings/${result.bookingId}` : '/app/bookings'}>
          <Button variant="primary">{result?.bookingId ? 'Xem lịch thay pin' : 'Xem danh sách lịch'}</Button>
        </Link>
      </div>
    </div>
  );
