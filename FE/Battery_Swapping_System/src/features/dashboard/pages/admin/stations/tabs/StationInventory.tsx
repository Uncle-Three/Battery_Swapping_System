...
type Action = 'INSPECTION_REQUIRED' | 'MAINTENANCE' | 'DELETE';
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Pin đang lưu tại trạm này</p>
        </div>
                    ...
                    <div className="inline-flex items-center gap-1">
                      ...
                      {b.operationalStatus !== 'RETIRED'
                        ...
                        <>
                          ...
                          <button
                            title="Xóa pin (Chuyển trạng thái ngừng sử dụng)"
                            disabled={busy}
                            onClick={() => openAction('DELETE', b)}
      ...
      {/* Modal Hành động (Bảo trì / Kiểm tra / Xóa mềm) */}
      <Modal
        isOpen={!!action}
        ...
        title={
          action === 'INSPECTION_REQUIRED'
            ...
            : action === 'MAINTENANCE'
            ? 'Gửi pin đi bảo trì'
            : 'Xác nhận đổi trạng thái xóa pin'
        }
        footer={
          <>
            ...
            <Button
              ...
              onClick={() => void submit()}
            >
              {action === 'DELETE' ? 'Xác nhận xóa' : 'Xác nhận'}
            </Button>
          </>
        }
      >
        <div className="space-y-4">
          {action === 'DELETE' && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3.5 text-xs text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/40 dark:text-amber-300 font-medium">
              <p className="font-bold text-amber-900 dark:text-amber-200">ℹ️ Thông báo xóa mềm (Soft Delete)</p>
              <p className="mt-0.5">
                Pin sẽ được chuyển sang trạng thái <strong>Ngừng sử dụng (RETIRED)</strong> để lưu giữ lịch sử chứ <strong>không bị xóa vĩnh viễn</strong> khỏi cơ sở dữ liệu.
              </p>
            </div>
          )}
          ...
          <label className="block text-sm font-bold text-slate-700 dark:text-slate-300">
            ...
            <textarea
              ...
              placeholder={action === 'DELETE' ? 'Nhập lý do chuyển sang trạng thái xóa...' : 'Nhập lý do thực hiện thao tác này...'}
              value={reason}
