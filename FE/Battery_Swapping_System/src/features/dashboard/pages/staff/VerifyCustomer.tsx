...
import { swapService, type StaffAssignmentState, type StaffBooking, type StaffStation, type StaffSwap } from '../../../../services/swapService';
import { getApiErrorMessage } from '../../../../services/apiClient';

const formatAssignedShift = (shift?: string | null) => {
  if (!shift) return 'Chưa đặt ca cụ thể';
  if (shift.includes('Tất cả')) return 'Tất cả các buổi (08:00 - 22:00)';

  return [
    shift.includes('Sáng') ? 'Sáng (08:00 - 12:00)' : '',
    shift.includes('Chiều') ? 'Chiều (12:00 - 18:00)' : '',
    shift.includes('Tối') ? 'Tối (18:00 - 22:00)' : '',
  ].filter(Boolean).join(' · ') || shift;
};
...
export const VerifyCustomer = () => {
  ...
  const [assignmentState, setAssignmentState] = useState<StaffAssignmentState>('UNASSIGNED');
  ...
  useEffect(() => {
    ...
    const loadContext = () => {
      swapService.context().then((data) => {
        ...
        setAssignmentState(data.assignmentState);
        if (!data.stations.length) setBooking(null);
      }).catch((cause) => {
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-amber-700 dark:text-amber-400">
          ...
          <p className="text-sm font-medium">
            {assignmentState === 'OUTSIDE_SHIFT'
              ? 'Bạn hiện đang ngoài ca làm việc được phân công. Danh sách xe sẽ xuất hiện khi ca làm bắt đầu.'
              : 'Bạn chưa được phân công vào trạm đang hoạt động.'}
          </p>
        </div>
      )}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1.5 flex items-center gap-1.5 text-sm font-semibold text-emerald-700 dark:text-emerald-400">
                  ...
                  Ca làm: {formatAssignedShift(selectedStation.assignedShift)}
                </p>
              </div>
            </div>
