...
import {
  ...
  ClipboardCheck, Gauge, History, LogOut, Menu, ScanLine, Car,
  Search, Settings, ShieldCheck, SlidersHorizontal, UserCheck, Users, Wrench, X,
} from 'lucide-react';
...
const roleNavigation: Partial<Record<string, NavGroup[]>> = {
  ...
  [UserRole.ADMIN]: [
    { label: 'Quản trị hệ thống', items: [
      { to: '/admin/dashboard', label: 'Bảng điều khiển', icon: Gauge },
      ...
      { to: '/admin/vehicle-transfers', label: 'Duyệt đơn chuyển nhượng chủ xe', icon: ShieldCheck },
...
const pageTitles: Record<string, string> = {
  ...
  'vehicle-transfers': 'Duyệt đơn chuyển nhượng chủ xe', vehicles: 'Quản lý xe',
...
export const DashboardLayout = () => {
  ...
  const unreadNotificationCount = useUnreadNotifications(Boolean(user));
  ...
  const notificationPath = user?.role === UserRole.ADMIN
    ? '/admin/notifications'
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <div className="flex h-full flex-col bg-slate-950 text-slate-300">
      ...
      <nav className="scrollbar-thin flex-1 overflow-y-auto px-3 py-5" aria-label="Điều hướng chính">
        <div className="space-y-1">
          {groups.flatMap((group) => group.items).map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} onClick={() => setMobileOpen(false)} title={collapsed && !mobile ? label : undefined} className={({ isActive }) => `group flex min-h-11 items-center rounded-xl text-sm font-semibold transition ${collapsed && !mobile ? 'justify-center px-2' : 'gap-3 px-3'} ${isActive ? 'bg-emerald-500 text-slate-950 shadow-lg shadow-emerald-500/10' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>
