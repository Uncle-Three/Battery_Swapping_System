...
import { BaySlotToggleModal } from '../../../components/admin/bay-slots/BaySlotToggleModal';
import type {
  BulkSlotPayload,
  PreviewSlot,
...
export const BaySlotManagementPage: FC = () => {
  ...
  const [values, setValues] = useState(() =>
    defaultValues(station.openingTime, station.closingTime, station.workingDays),
  );
  ...
  const reset = () => {
    const next = defaultValues(station.openingTime, station.closingTime, station.workingDays);
  ...
  const applyToggle = async () => {
    ...
    const nextStatus: SlotStatus = toggleSlot.status === 'OFF' ? 'AVAILABLE' : 'OFF';
    try {
        ...
        current.map((slot) =>
          slotKey(slot) === slotKey(toggleSlot)
            ? {
                ...slot,
                status: nextStatus,
                reason: nextStatus === 'OFF' ? toggleReason.trim() || undefined : undefined,
              }
            : slot,
        ),
      );
      setToggleSlot(null);
      setToggleReason('');
      setNotice(nextStatus === 'OFF' ? 'Đã tắt khung giờ.' : 'Đã bật lại khung giờ.');
    } catch (cause) {
