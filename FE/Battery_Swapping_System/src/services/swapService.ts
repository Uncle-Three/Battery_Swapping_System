...
type StaffVehicle = { name: string; plateNumber: string; batteryType?: string | null; vinNumber?: string | null; brand?: string | null; model?: string | null; manufactureYear?: number | null; color?: string | null; batteryAssignments?: Array<{ battery: { id: string; batteryCode?: string | null; qrCodeValue?: string | null; serialNumber?: string | null; soh?: number | null; estimatedSoH?: number | null; healthClassification?: string | null } }> };
export type StaffBayBooking = { id: string; stationId: string; serviceBayId: string; status: string; scheduledStart?: string; scheduledEnd?: string; mandatory?: boolean; reason?: string | null; user: { fullName: string; email: string; phone?: string }; vehicle?: StaffVehicle; battery?: { serialNumber: string } };
...
export type StaffStation = { id: string; name: string; address: string; assignedShift?: string | null; serviceBays: StaffServiceBay[] };
export type StaffBooking = { id: string; stationId: string; serviceBayId?: string | null; status: string; scheduledStart?: string; scheduledEnd?: string; costEstimate?: number | null; reason?: string | null; mandatory?: boolean; user: { fullName: string; email: string; phone?: string }; vehicle?: StaffVehicle; battery?: { serialNumber: string }; serviceBay?: StaffServiceBay | null; station?: StaffStation };
export type StaffSwap = { id: string; workflowStatus: string; createdAt: string; paymentUrl?: string; nextActions?: string[]; booking?: StaffBooking; vehicle?: StaffVehicle; batteryIn?: { serialNumber: string; soc: number; soh: number; temperature: number; voltage: number }; batteryOut?: { serialNumber: string; soc: number; soh: number }; inspection?: unknown; stepHistory?: Array<{ id: string; fromStatus?: string; toStatus: string; createdAt: string; data?: { vehicleBatterySerial?: string; [key: string]: unknown } | null }> };
export type StaffAssignmentState = 'ACTIVE_SHIFT' | 'OUTSIDE_SHIFT' | 'UNASSIGNED';
...
export const swapService = {
  context: async () => unwrapData<{ stations: StaffStation[], activeSwap: StaffSwap | null, assignmentState: StaffAssignmentState }>(await apiClient.get(API_ENDPOINTS.SWAP.CONTEXT)),
  ...
  remove: async (id: string, serialNumber: string, soc: number, soh?: number) => unwrapData<StaffSwap>(await apiClient.post(API_ENDPOINTS.SWAP.REMOVE(id), { serialNumber, soc, ...(soh !== undefined ? { soh } : {}) })),
  ...
  requestDirectPayment: async (id: string, paymentMethod: 'VNPAY' | 'CASH' = 'VNPAY') => unwrapData<StaffSwap>(await apiClient.post(API_ENDPOINTS.SWAP.COLLECT_PAYMENT(id), { paymentMethod })),
