export const getBookingStatusLabel = (status: string) => {
  switch (status) {
    ...
    case 'CHECKED_IN': return { label: 'Đã check-in', color: 'bg-indigo-100 text-indigo-700' };
    case 'VERIFIED': return { label: 'Đã xác minh', color: 'bg-cyan-100 text-cyan-700' };
    ...
    case 'EXPIRED': return { label: 'Hết thời gian giữ chỗ', color: 'bg-red-100 text-red-700' };
