...
import { ArrowLeft, RefreshCw, CheckCircle, XCircle, HelpCircle, FileText } from "lucide-react";
...
export default function VehicleTransferReview() {
  ...
  const canReject = ["PENDING", "UNDER_REVIEW"].includes(request.status);
  const canRequestInfo = request.status === "UNDER_REVIEW";
          ...
          <div className="app-panel p-6">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              {[
                ...
                { label: "Trạng thái sở hữu", value: request.vehicle?.ownershipStatus },
          ...
          <div className="app-panel p-6">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={doc.label} className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
                  <div className="flex items-center gap-3">
                    <FileText className="h-4 w-4 text-slate-500" />
                    <div>
                      <p className="text-sm font-bold text-slate-900 dark:text-white">{doc.label}</p>
                      {doc.required && <p className="text-xs text-slate-400">Bắt buộc</p>}
                    </div>
                  </div>
                  {doc.url ? (
                    <a href={doc.url} target="_blank" rel="noopener noreferrer" className="text-sm font-bold text-emerald-600 hover:underline dark:text-emerald-400">
                      Mở tài liệu →
                    </a>
                  ) : (
                    <span className={`text-xs font-semibold ${doc.required ? "text-red-500" : "text-slate-400"}`}>
                      {doc.required ? "⚠️ Chưa có" : "Không áp dụng"}
                    </span>
                  )}
                </div>
              ))}

              {request.additionalDocumentUrls.length > 0 && (
                <div className="pt-2">
                  <p className="text-xs font-bold text-slate-500 mb-2">Tài liệu phụ</p>
                  {request.additionalDocumentUrls.map((url, i) => (
                    <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="block text-xs font-semibold text-emerald-600 hover:underline truncate dark:text-emerald-400">
                      Tài liệu bổ sung {i + 1}
                    </a>
                  ))}
                </div>
              )}
