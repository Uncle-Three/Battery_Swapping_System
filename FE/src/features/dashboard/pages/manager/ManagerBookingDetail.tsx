...
export const ManagerBookingDetail = () => {
  ...
  const load = useCallback(async () => { 
    try { 
      ...
      setSlotId((value) => value || stationSlots[0]?.id || ''); 
      if (booking.scheduledStart) {
        const d = new Date(booking.scheduledStart);
        setScheduledStart(`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}T${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`);
      }
    } catch (cause) { 
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end justify-between">
          <div>
            <p className="eyebrow flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-slate-500">
              <MapPin className="h-3.5 w-3.5" /> {item.station.name}
            </p>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-xl border border-slate-100 p-4 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20">
                ...
                {item.serviceBay && <p className="mt-2 font-extrabold text-slate-900 dark:text-white">Khoang: {item.serviceBay.bayName}</p>}
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 h-fit">
              ...
              <div className="mt-5 rounded-xl bg-slate-50 p-5 dark:bg-slate-800/50">
                <p className="font-extrabold text-lg text-slate-900 dark:text-white">{item.user.fullName}</p>
                <div className="mt-3 space-y-2">
                  ...
                  <div className="flex justify-between items-center pt-1">
                    ...
                    <span className="text-sm font-bold text-slate-800 dark:text-slate-200 break-all ml-4 text-right">{item.user.email}</span>
                  </div>
                </div>
              </div>
            </section>
          ...
          {item.status === 'PENDING_APPROVAL' && (
            <section className="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-6 shadow-sm dark:border-emerald-900/30 dark:bg-emerald-950/10">
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-xl border border-amber-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2"><Clock className="h-4 w-4 text-amber-600" /> Đề xuất Đổi lịch</h3>
                  
                  <label className="mt-3 block text-xs font-bold uppercase tracking-wider text-slate-500">Khung giờ đề xuất</label>
                  <select className={inputClass} value={slotId} onChange={(event) => setSlotId(event.target.value)}>
                    {slots.map((slot) => <option key={slot.id} value={slot.id}>Khung {slot.slotNumber}</option>)}
                  </select>
                  
                  <label className="mt-3 block text-xs font-bold uppercase tracking-wider text-slate-500">Thời gian đề xuất</label>
                  <input type="datetime-local" step="3600" className={inputClass} value={scheduledStart} onChange={(event) => setScheduledStart(event.target.value)} />
                  
                  <label className="mt-3 block text-xs font-bold uppercase tracking-wider text-slate-500">Ghi chú đổi lịch</label>
                  <textarea rows={2} placeholder="Lý do đổi lịch..." className={inputClass} value={rescheduleReason} onChange={(event) => setRescheduleReason(event.target.value)} />
                  
                  <Button className="mt-4 w-full justify-center" variant="outline" disabled={!slotId || rescheduleReason.trim().length < 3} loading={busy} onClick={() => { const start = new Date(scheduledStart); void act(() => managerBookingService.reschedule(item.id, { slotId, scheduledStart: start.toISOString(), scheduledEnd: new Date(start.getTime() + 3_600_000).toISOString(), reason: rescheduleReason }), 'Đã gửi đề xuất đổi lịch.'); }}>Gửi Đề xuất</Button>
                </div>
              </div>
            </section>
          )}
              ...
              {item.transactions.map(tx => (
                <div key={tx.id} className="mt-4">
                  ...
                  <div className="flex flex-col gap-4 relative">
                    {tx.stepHistory?.length ? tx.stepHistory.map((step, idx) => (
                      <div key={step.id} className="relative pl-6">
                        ...
                        {idx !== tx.stepHistory!.length - 1 && <div className="absolute left-[7px] top-5 bottom-[-16px] w-px bg-slate-200 dark:bg-slate-800" />}
